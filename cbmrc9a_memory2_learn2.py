...
def inv_run_network(mode):
    global Hx, Hs, Hp2, Y, Yx, Ys, Yp, Y, Us, Ds,Rs
    Hp2 = np.zeros((c.MM, c.Nh))
    Hx = np.zeros((c.MM*c.NN, c.Nh))
    Hs = np.zeros((c.MM*c.NN, c.Nh))
    hsign = np.zeros(c.Nh)
    hx = np.zeros(c.Nh)
    ...
    for n in tqdm(range(c.NN * c.MM)):
        ...
        if rs_prev==0 and rs==1:
            ...
            hp = 2*hc/c.NN-1 # デコード、カウンタの値を連続値に変換
            # Wr = np.dot(Hp[c.MM0+m+1:, :].T,np.linalg.pinv(hp).T).T
            # print(Wr.shape)
            hc = np.zeros(c.Nh) #カウンタをリセット
            ht = 2*hs-1 #リファレンスクロック同期用ラッチ動作をコメントアウト
...
def inv_train_network():
    global Wi

    inv_run_network(1) # run netwrok with teacher forcing
    ...
    if c.lambda0 == 0:
        ...
        Wi = np.dot(G.T,np.linalg.pinv(M).T).T
        #print("a")
    else:
        E = np.identity(c.Nh)
        TMP1 = np.linalg.inv(M.T@M + c.lambda0 * E)
...
def calc_mc(Dp,Yp):
    for k in range(c.delay):
            corr = np.corrcoef(np.vstack((Dp.T[k, k+c.MM0:], Yp.T[k, k+c.MM0:])))   #相関係数
