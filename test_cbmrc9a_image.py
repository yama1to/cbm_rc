...
### 共通設定
from cbmrc9a_image import Config
config = Config()
common.config  = config
common.prefix  = "data%s_cbmrc9a_image" % common.string_now() # 実験名（ファイルの接頭辞）
common.dir_path= "data/data%s_cbmrc9a_image" % common.string_now() # 実験データを出力するディレクトリのパス
common.exe     = "python cbmrc9a_image.py " # 実行されるプログラム
common.columns =['dataset','seed','id','Nh','alpha_i','alpha_r',"alpha_s","Temp",'beta_i','beta_r','lambda0','WER',"cnt_overflow"]
common.parallel= 16
common.setup()
...
def rs1():
    ...
    df = df[['alpha_r','alpha_i','WER']] # 指定した列のみでデータフレームを構成する
...
def optimize():
    ...
    opt.append("Temp",value=1,min=1,max=10,round=1)
...
def gridsearch(X1,min=0,max=1,num=41,samples=10):
    ...
    plot1(x,ymean,ystd,ymin,ymax,color=cmap(1),label="cnt_overflow")
    plt.ylabel("cnt_overflow")
    plt.grid(linestyle="dotted")
...
def gs2():
    ...
    gridsearch("lambda0",min=0.0001,max=1,num=41,samples=ns)
gs2()
