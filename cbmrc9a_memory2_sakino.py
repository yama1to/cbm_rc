...
def deep_weight():
    ...
    Wr1 = generate_random_matrix(c.Nh-10,c.Nh-10,c.alpha_r,c.beta_r,distribution="one",normalization="sr",diagnal=1)

    Wr[10:,10:] = Wr1

    x = np.zeros((c.Nh-10)*10)
    x[x.size//2:]=1
    x[:x.size//2]=-1
    np.random.shuffle(x)
    x = x.reshape((c.Nh-10,10))
...
def generate_weight_matrix():
    ...
    if c.plot:
        ...
        G = nx.from_numpy_array(Wr)
        draw_network(G,k_over_2=0)
        plt.show()
