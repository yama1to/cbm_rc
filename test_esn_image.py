...
### 共通設定
from esn_image import Config
config = Config()
common.config  = config
common.prefix  = "data%s_esn_image" % common.string_now() # 実験名（ファイルの接頭辞）
common.dir_path= "data/data%s_esn_image" % common.string_now() # 実験データを出力するディレクトリのパス
common.exe     = "python esn_image.py " # 実行されるプログラム
common.columns =['dataset','seed','id','Nh','alpha_i','alpha_r','alpha_b',"alpha0",'beta_i','beta_r','beta_b','lambda0','WER']
common.parallel= 32
common.setup()
...
def gs2():
    ...
    gridsearch("lambda0",min=0.0001,max=1000,num=41,samples=ns)
gs2()
