...
def run_network(mode):
    ...
    count =0
    m = 0
    hp1 = np.zeros(int(c.Nh*c.alpha0))
    for n in tqdm(range(c.NN * c.MM)):
        ...
        if n%c.NN==0:
            hp1 = (Wr@(2*p2s(theta,hp)-1))[:int(c.Nh*c.alpha0)]
        elif n%c.NN==2:
            tmp = Wr@(2*p2s(theta,hp)-1) # リカレント結合
            tmp[:int(c.Nh*c.alpha0)] = hp1
            sum += tmp
