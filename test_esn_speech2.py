...
### 共通設定
from esn_speech2 import Config

config = Config()
common.config  = config
common.prefix  = "data%s_esn_speech2" % common.string_now() # 実験名（ファイルの接頭辞）
common.dir_path= "data/data%s_esn_speech2" % common.string_now() # 実験データを出力するディレクトリのパス
common.exe     = "python esn_speech2.py " # 実行されるプログラム
...
def optimize():
    ...
    #opt.append("alpha_i",value=10**4,min=0.01,max=10**5,round=2)
    opt.append("alpha0",value=1,min=0.01,max=1,round=2)
    #opt.append("lambda0",value=0,min=0.01,max=10,round=2)
...
def gs2():
    ns=3
    gridsearch("Nh",min=100,max=1000,num=20,samples=ns)
    #gridsearch("Temp",min=0.01,max=10,num=100,samples=ns)
    gridsearch("beta_r",min=0.05,max=1,num=30,samples=ns)
