...
def run_network(mode):
    ...
    group = [0,int(c.Nh//3),int(c.Nh//3*2),c.Nh]

    us = np.zeros((3))

    for n in tqdm(range(c.NN * c.MM)):
        theta = np.mod(n/c.NN,1) # (0,1)
        theta1 = np.mod(n/c.NN*2,1)
        theta2 = np.mod(n/c.NN/2,1)
        thetas = [theta1,theta,theta2]
        
        rs_prev = rs
        hs_prev = hs.copy()

        for i in range(3):
            rs[i] = p2s(thetas[i],0)# 参照クロック
            us[i] = p2s(thetas[i],Up[m]) # エンコードされた入力
            # ds = p2s(thetas[i],Dp[m]) #
            # ys = p2s(thetas[i],yp)

            sum = np.zeros(c.Nh)
        #sum += c.alpha_s*rs # ラッチ動作を用いないref.clockと同期させるための結合
        for i in range(3):
            tmp = c.alpha_s*(hs-rs[i])*ht # ref.clockと同期させるための結合
            #print(Wi[group[i]:group[i+1]].shape,us[i].shape,Wi[group[i]:group[i+1]]@(2*us[i]-1).shape)
            tmp += Wi@[2*us[i]-1] # 外部入力
            tmp += Wr@(2*hs-1).T # リカレント結合
            sum[group[i]:group[i+1]] = tmp[group[i]:group[i+1]]
        ...
        for i in range(3):
            c.dt = 1/128/(i+1)
            hx[group[i]:group[i+1]] +=hsign[group[i]:group[i+1]]*(1.0+np.exp(hsign[group[i]:group[i+1]]*sum[group[i]:group[i+1]]/c.Temp))*c.dt

        hs = np.heaviside(hx+hs-1,0)
        hx = np.fmin(np.fmax(hx,0),1)
        for i in range(3):
            hc[(hs_prev[i] == 1)& (hs[i]==0)] = count
        
        
        # ref.clockの立ち上がり
        for i in range(3):
            if rs_prev[i]==0 and rs[i]==1:
                hp[group[i]:group[i+1]] = 2*hc[group[i]:group[i+1]]/c.NN-1 # デコード、カウンタの値を連続値に変換
                
                ht[group[i]:group[i+1]] = 2*hs[group[i]:group[i+1]]-1 #リファレンスクロック同期用ラッチ動作をコメントアウト
