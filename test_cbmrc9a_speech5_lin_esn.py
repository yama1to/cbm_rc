...
### 共通設定
from cbmrc9a_speech5_lin_esn import Config
config = Config()
common.config  = config
common.prefix  = "data%s_cbmrc9a_speech5_lin_esn" % common.string_now() # 実験名（ファイルの接頭辞）
common.dir_path= "data/data%s_cbmrc9a_speech5_lin_esn" % common.string_now() # 実験データを出力するディレクトリのパス
common.exe     = "python cbmrc9a_speech5_lin_esn.py " # 実行されるプログラム
common.columns =['dataset','seed','id','NN','Nh','alpha_i','alpha_r','alpha_b','alpha_s',"alpha0",'beta_i','beta_r','beta_b','Temp','lambda0','cnt_overflow','WER']
common.parallel= 32
common.setup()
...
def optimize():
    ...
    opt.append("alpha_r",value=0.6,min=0.0,max=1,round=2)
    opt.append("alpha_s",value=1,min=0.0,max=2,round=2)
    opt.append("alpha0",value=0.1,min=0.0,max=1,round=2)
