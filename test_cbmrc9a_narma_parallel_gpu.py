...
### 共通設定
from cbmrc9a_narma_parallel_gpu import Config
config = Config()
common.config  = config
common.prefix  = "data%s_cbmrc9a_narma_parallel_gpu" % common.string_now() # 実験名（ファイルの接頭辞）
common.dir_path= "data/data%s_cbmrc9a_narma_parallel_gpu" % common.string_now() # 実験データを出力するディレクトリのパス
common.exe     = "python cbmrc9a_narma_parallel_gpu.py " # 実行されるプログラム
common.columns =['dataset','seed','id','NN','Nh','alpha_i','alpha_r','alpha_b','alpha_s','beta_i','beta_r',\
    'beta_b','Temp','lambda0','delay','cnt_overflow',"RMSE",'NRMSE',"NMSE",\
        "parallel"]
common.parallel= 8
common.setup()
...
def optimize():
    ...
    opt.minimize(target="NMSE",iteration=10,population=16,samples=3)
