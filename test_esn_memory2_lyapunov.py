...
### 共通設定
from esn_memory2_lyapunov import Config
config = Config()
common.config  = config
common.prefix  = "data%s_esn_memory2_lyapunov" % common.string_now() # 実験名（ファイルの接頭辞）
common.dir_path= "data/data%s_esn_memory2_lyapunov" % common.string_now() # 実験データを出力するディレクトリのパス
common.exe     = "python esn_memory2_lyapunov.py " # 実行されるプログラム
common.columns=['dataset','seed','id','Nh','alpha_i','alpha_r','alpha0','beta_i','beta_r',
'lambda0',"delay",'MC',"MC1",'MC2','MC3','MC4',"lyapunov"]
common.parallel= 100
common.setup()
...
def rs1():
    ...
    df = df[['alpha_r','alpha_i','cnt_overflow','lyapunov']] # 指定した列のみでデータフレームを構成する
...
def optimize():
    ...
    #opt.maximize(target="MC",iteration=20,population=16,samples=3)
    opt.minimize(target="lyapunov",iteration=20,population=16,samples=3)
...
def gridsearch(X1,min=0,max=1,num=41,samples=10):
    ...
    plt.subplot(3,1,2)
    x,ymean,ystd,ymin,ymax = vs.analyze(df,X1,"lyapunov")
    plot1(x,ymean,ystd,ymin,ymax,color=cmap(1),label="lyapunov")
    plt.ylabel("lyapunov")
    plt.grid(linestyle="dotted")
