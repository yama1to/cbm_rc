...
### 共通設定
from cbmrc9a_santafe import Config
config = Config()
common.config  = config
common.prefix  = "data%s_cbmrc9a_santafe" % common.string_now() # 実験名（ファイルの接頭辞）
...
common.columns =['dataset','seed','id','NN','Nh','alpha_i','alpha_r','alpha_b','alpha_s','beta_i','beta_r','beta_b','Temp','lambda0',"RRMSE1",'cnt_overflow','NRMSE',"NRMSE2"]
common.parallel= 32
common.setup()
...
def optimize():
    ...
    opt.append("alpha_r",value=2,min=1,max=10,round=2)
    ...
    opt.minimize(target="NRMSE",iteration=50,population=50,samples=4)
