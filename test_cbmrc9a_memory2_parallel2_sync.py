...
### 共通設定
from cbmrc9a_memory2_parallel2_sync import Config
config = Config()
common.config  = config
common.prefix  = "data%s_cbmrc9a_memory2_parallel2_sync" % common.string_now() # 実験名（ファイルの接頭辞）
common.dir_path= "data/data%s_cbmrc9a_memory2_parallel2_sync" % common.string_now() # 実験データを出力するディレクトリのパス
common.exe     = "python cbmrc9a_memory2_parallel2_sync.py " # 実行されるプログラム
common.columns=['dataset','seed','id','NN','Nh',\
    'alpha_i','alpha_r','alpha_s',\
    ...
    'beta_i','beta_r',\
    'beta_i2','beta_r2',\
    'beta_i3','beta_r3','beta_b','Temp','lambda0',"delay",'RMSE1','RMSE2','cnt_overflow','MC']
common.parallel= 90
common.setup()
...
def optimize():
    ...
    opt.append("beta_i2",value=0.01,min=0.0,max=1,round=2)
    opt.append("alpha_i2",value=1,min=0.00,max=1,round=2)
    opt.append("alpha_r2",value=1,min=0.,max=1,round=2)
    opt.append("alpha_s2",value=1,min=0,max=2,round=2)

    opt.append("beta_r3",value=0.01,min=0.0,max=1,round=2)
    opt.append("beta_i3",value=0.01,min=0.0,max=1,round=2)
    opt.append("alpha_i3",value=1,min=0.00,max=1,round=2)
    opt.append("alpha_r3",value=1,min=0.,max=1,round=2)
    opt.append("alpha_s3",value=1,min=0,max=2,round=2)
