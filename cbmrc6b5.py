...
def run_network(mode):
    ...
    for n in range(NN*MM):
        ...
        #update_s(hx,hs,Nh)
        hx,hs,is_update = update2_s(hx,hs,Nh)
        #hs=np.heaviside(hx-1,1)
        #hs=np.heaviside(hx,0)
        #hx=np.fmin(hx,1)
        #hx=np.fmax(hx,0)
...
def execute2():
    ...
    MM1=40
    MM=MM1
    Dp = np.zeros(MM*2).reshape((MM,2))
    Up = np.zeros(MM*2).reshape((MM,2))
    train_network()
    plot3()
