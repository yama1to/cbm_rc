...
### 共通設定
from cbmrc9a_memory2_gpu import Config
config = Config()
common.config  = config
common.prefix  = "data%s_cbmrc9a_memory2_gpu" % common.string_now() # 実験名（ファイルの接頭辞）
common.dir_path= "data/data%s_cbmrc9a_memory2_gpu" % common.string_now() # 実験データを出力するディレクトリのパス
common.exe     = "python cbmrc9a_memory2_gpu.py " # 実行されるプログラム
common.columns=['dataset','seed','id','NN','Nh','alpha_i','alpha_r','alpha_b','alpha_s',"alpha1",'beta_i','beta_r','beta_b',"over",
...
def optimize():
    ...
    opt.append("beta_r",value=0.01,min=0.0,max=1,round=5)
    ...
    opt.append("over",value=1,min=0,max=3,round=2)
...
def gs2():
    ns=3
    gridsearch("Nh",min=50,max=1000,num=41,samples=ns)
