...
def run_network(mode):
    ...
    for n in tqdm(range(c.NN * c.MM)):
        ...
        #hx = hx + hsign*(1.0+c.alpha1*np.exp(hsign*sum/c.Temp))*c.dt
        hx = hx + hsign*(1.0+c.alpha1**(hsign*sum/c.Temp))*c.dt
        #print(np.exp(hsign*sum/c.Temp))
        hs = np.heaviside(hx+hs-1,0)

        # hx = hx_prev + hsign*(1.0+0.1*np.exp(hsign*(sum+c.alpha_s*(hs-rs)*ht)/c.Temp))*c.dt
