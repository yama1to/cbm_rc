...
### 共通設定
from esn_speech4 import Config

config = Config()
common.config  = config
common.prefix  = "data%s_esn_speech4" % common.string_now() # 実験名（ファイルの接頭辞）
common.dir_path= "data/data%s_esn_speech4" % common.string_now() # 実験データを出力するディレクトリのパス
...
def optimize():
    ...
    opt.append("alpha_r",value=0.9,min=7,max=1,round=2)
...
def gs2():
    ...
    gridsearch("alpha_i",min=1,max=20000,num=501,samples=ns)
