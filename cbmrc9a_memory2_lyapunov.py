...
def run_network(mode):
    ...
    for n in tqdm(range(c.NN * c.MM)):
        ...
        sum2 = np.zeros(c.Nh)
        #sum += c.alpha_s*rs # ラッチ動作を用いないref.clockと同期させるための結合
        sum2 += c.alpha_s*(hs2-rs2)*ht2 # ref.clockと同期させるための結合
        sum2 += Wi@(2*us-1) # 外部入力
        sum2 += Wr@(2*hs2-1) # リカレント結合
        ...
        hc2[(hs_prev2 == 1)& (hs2==0)] = count2
        
        
        # ref.clockの立ち上がり
        if rs_prev2==0 and rs2==1:
